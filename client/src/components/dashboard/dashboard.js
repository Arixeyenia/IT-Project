...
function DisplayPortfolioItem(arrayOfPortfolioObjects, deletePortfolio) {
  return arrayOfPortfolioObjects.map((object, i) => (
    <GridListTile className='portfolioListItem' key={object.portfolio._id}>
      <Card raised={true} className='portfolioCard'>
        ...
        <CardContent className='overlayPortfolioItem'>
          <Typography variant='body1' className='fontg6'>
...
function IndividualMenu(props) {
  ...
  return (
    <Box className='card-action-section'>
      ...
      >
        <MenuItem
          onClick={() => history.push('/view/' + props.object.portfolio._id)}
        >
          View
        </MenuItem>
        <MenuItem
          onClick={() => history.push('/edit/' + props.object.portfolio._id)}
        >
          Edit
        </MenuItem>
        <MenuItem
          onClick={() => {
            props.deletePortfolio(props.object.portfolio._id);
          }}
        >
          Delete
        </MenuItem>
        <MenuItem
          onClick={(event) => openPopover(event, props.object.portfolio._id)}
        >
          Get link
        </MenuItem>
      </Menu>
      ...
      >
        ...
        <Button
          ...
          color='primary'
          onClick={() => {
            copyClipboardLink('text-' + props.i);
          }}
        >
          Copy to clipboard
        </Button>
