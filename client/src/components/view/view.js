    ...
    <Grid item xs={12 / rowLengths[object.row]}>
      <Card className={classes.cardRoot} variant='outlined'>
        {object.mediaType === 'image' && (
          <CardMedia className={classes.media} image={object.mediaLink} />
        )}
        <CardHeader
        ...
        <CardActions>
          <Button
            size='small'
            onClick={() => {
              if (!/^(f|ht)tps?:\/\//i.test(object.linkAddress)) {
                history.push('/view/' + portfolioID + '/' + object.linkAddress);
              } else {
                window.location.href = object.linkAddress;
              }
